...
const SkillsSection = () => {
  ...
  return (
    <section
      ref={sectionRef}
      id="skills"
      className={`px-6 py-24 ${isDarkMode ? "bg-gray-950 text-white " : "bg-gray-50 text-gray-900"} relative overflow-hidden`}
    >
      {/* background elements */}
      <motion.div
      ...
      <div className='max-w-6xl mx-auto relative z-10'>
        ...
        >
          {
            SKILLS_CATEGORY.map((category, index) => (
              <motion.div
                key={index}
                variants={itemVarients}
                className={`p-8 rounded-2xl border ${isDarkMode ? "bg-gray-900/50 broder-gray-800 backdrop-blur-sm" : "bg-white-80 border-gray-200 backdrop-blur-sm "
                  }`}>
                {/* category header */}
                <div className='flex items-center mb-6'>
                  ...
                  <div>
                    ...
                    <p className={`text-sm ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>{category.descreption}</p>
                  </div>
                </div>
                    ...
                    category.skills.map((skill, ind) => (
                      <div key={ind} className='group'>
                        <div className='flex justify-between items-center mb-2'>
                          ...
                          <span className={`text-xs ${isDarkMode ? "text-gray-500" : "text-gray-600"}`}>{skill.level}</span>
                        </div>
                        <div className={`h-2 rounded-full overflow-hidden ${isDarkMode ? "bg-gray-800" : "bg-gray-200"}`}>
                          <motion.div variants={skillBarVAriants}
                            initial="hidden"
                            animate={isInView ? "visible" : "hidden"}
                            custom={skill.level}
        ...
        <motion.div initial="hidden"
          ...
          className='mt-16'
        >
          <motion.div variants={itemVarients} className='text-center mb-8'>
            <h3 className='text-xl font-medium mb-4'>Also Working With</h3>
          </motion.div>
          <motion.div variants={itemVarients} className='flex flex-wrap justify-center gap-3'>
            {
              TECH_STACK.map((tech, ind) => (
                <motion.span key={ind}
        ...
        >
          {
            STATS.map((stat, ind) => (
              <motion.div key={ind}
              variants={itemVarients} className='text-center'>
                <div className='text-2xl md:text-3xl font-light text-blue-500 mb-2'>
                  {stat.number}
                </div>
                <div className={`text-sm ${isDarkMode ? 'text-gray-499' : 'text-gray-600'}`}>
                  {stat.label}
                </div>
