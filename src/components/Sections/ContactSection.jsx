...
import SuccessModel from '../compo/SuccessModel'
  ...
  return (
    ...
    >
      ...
      <div className='max-w-6xl mx-auto relative z-10'>
        ...
        <div className='grid lg:grid-cols-2 gap-16 items-start'>
          ...
          >
              ...
              <div className='grid grid-cols-2 gap-4'>
                    {
                      SOCIAL_LINKS.map((social)=>(
                        ...
                        whileTap={{scale:0.95}}
                        className={`flex items-center p-4 space-x-3 rounded-xl border transition-all duration-300 ${isDarkMode ? 'bg-gray-800/50 border-gray-700 hover:border-gray-600' : 'bg-white/80 border-gray-200 hover:border-gray-300'} ${social.bgColor} ${social.color}`}
                        >
                          <social.icon size={20}/>
            ...
            >
              ...
              <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>I'm currently available for freelance projects and full-time opportunities</p>
            </motion.div>
          </motion.div>
        </div>
        ...
        >
          <motion.div variants={itemVarients}
           className={`max-w-2xl mx-auto p-8 rounded-2xl border ${isDarkMode ? 'bg-gray-800/30 border-grat-700' : 'bg-gray-50/50 border-gray-200'}`}>
            <h3 className='text-xl font-medium mb-4'>Prefer a quick call?</h3>
            <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-600'} mb-6`}>Sometimes a conversation is worth a thousand messages. Feel free to schedule a call to discuss your project.</p>
            <motion.button
            whileHover={{y:-2, scale:1.05}}
            whileTap={{scale:0.98}}
            className={`px-6 py-3 rounded-full border font-medium transitionall duration-300 ${isDarkMode ? 'border-gray-600 hover:border-blue-500 hover:text-blue-400' : 'border-gray-300 hover:border-blue-500 hover:text-blue-600'}`}
